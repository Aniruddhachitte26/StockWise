...
//   function getVerificationStatus(verified) {
//     if (verified === true || verified === 'true' || verified === 1 || verified === '1') {
//       return 'approved';
//     } else if (verified === false || verified === 'false' || verified === 0 || verified === '0') {
//       return 'rejected';
//     } else {
//       return 'pending'; // null, undefined, or any other value
//     }
//   }
...
//   // Pending means neither approved nor rejected
//   filteredUsers = fetchedUsers.filter(user => 
//     user.verified !== true && user.verified !== false);
...
//       // Set verified to true for approve, false for reject
//       const verifiedValue = action === 'approve' ? true : false;
      
//       console.log("Verifying user:", selectedUser._id, "Action:", action, "Setting verified to:", verifiedValue);
...
//         {
//           _id: selectedUser._id,
//           verified: verifiedValue // true for approve, false for reject
//         },
//         { headers: { Authorization: `Bearer ${token}` } }
//       );
...
//       // Show success message
//       setSuccessMessage(
//         `User ${selectedUser.fullName} has been ${action === 'approve' ? 'approved' : 'rejected'} successfully.`
//       );
...
//                           <Badge 
//   bg={
//     user.verified === true || user.verified === 'true' ? 'success' : 
//     user.verified === false || user.verified === 'false' ? 'danger' : 
//     'warning'
//   }
//   style={{ 
//     backgroundColor: 
//       user.verified === true || user.verified === 'true' ? 'var(--accent)' : 
//       user.verified === false || user.verified === 'false' ? 'var(--danger)' : 
//       'var(--warning, #ffc107)'
//   }}
// >
//   {
//     user.verified === true || user.verified === 'true' ? 'Approved' : 
//     user.verified === false || user.verified === 'false' ? 'Rejected' : 
//     'Pending'
//   }
// </Badge>
//                           </td>
...
const UserVerificationPage = () => {
  ...
  const handleVerifyUser = async () => {
    ...
    try {
      ...
      // Show success message
      setSuccessMessage(`User ${selectedUser.fullName} has been ${verificationStatus} successfully.`);
